...
| PL-005 | Beratung | Keine Beratungs-/Telefon-/Maßprüfungs-/Verlegeanfrage im Warenkorb; Cart-Notiz abgeschaltet (`show_cart_note: false`) | `snippets/cart-summary.liquid`, Bestellungen ohne customAttributes | M5 | OFFEN |
| PL-007 | Express-Checkout | Dynamic Checkout (Shop Pay/Apple/Google Pay) auf PDP und im Cart aktiv → jede Pflichtabfrage im Cart umgehbar | `blocks/accelerated-checkout.liquid:23`, `cart-summary.liquid:311-318` | M5 | OFFEN |
...
| PL-011 | Konsistenz | Versandschwelle: `tp_versand_frei_ab` im Repo leer, `[versandfrei]`-Platzhalter 11×, hart „50 €" 2× – Live zeigt 50 €, Repo-Stand rendert „ab € Bestellwert" | `settings_data.json`, `header-group.json:468`, `tp-preisangabe.liquid:5` | M2 | OFFEN |
| PL-012 | Vertrauen | Google-Bewertung: Theme-Einstellungen `tp_google_wert/anzahl` im Repo leer; Wert nur im Editor des Live-Themes → Preview/Arbeitstheme ohne Beleg | `config/settings_data.json` | M2 | OFFEN |
| PL-013 | Produktidentität | `variant.sku` kundensichtbar (`blocks/sku.liquid`, JSON-LD) mit Lieferanten-Artikelmuster | 01-CURRENT-STATE 1.1 | M1 | OFFEN (prüfen, ob Block in Templates aktiv) |
