...
| Shopify | `shpat_`-Token mit `read_orders`/`write_orders`/`read_customers` bereitstellen (#34) | 🟠 | Ahmet | `.env.local` (nie Repo) | `router:status` | Blocker fuer alles Weitere |
...
| Daten | Metafeld-Definitionen `einkauf.*`, Metaobjekte `tp_lieferant`, `tp_einkauf`, Order-Namespace `ops` anlegen | ⬜ | Fable via MCP | `domains/shopify/einkauf-metafelder.json` | Definition per Query gegenpruefen | nach D2-Freigabe |
...
| Fulfillment | Interne Bestellmail im Admin einsetzen (5 Schritte) | 🟠 | Ahmet (Admin) | – | Testbestellung | keine API dafuer |
...
| Repo | Sync-Bugs OPS-010/011 beheben oder Skripte stilllegen | ⬜ | Fable (Klasse B) | `workflow/sync-grosshandel.mjs` | `workflow:test` | – |
