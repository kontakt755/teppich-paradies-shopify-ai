...
- Teppiche nach Mass: der ab-Preis zeigt bei aktiver Aktion den alten ab-Preis durchgestrichen (gleiche Formel mit `compare_at_price` der Variante; kein Streichpreis, wenn schon beim alten Preis der Mindestpreis greift - sonst entstuende ein Cent-"Rabatt").
- Erinnerung zum Aktionsende: geplante Aufgabe `rabattaktion-welle1-beenden` (einmalig 2026-10-19 08:00, laeuft nur bei geoeffneter App) + Issue #413.
- Zum Testen von Preisanzeigen mit echten Daten: unlisted Testprodukt `test-callista-teppich-mit-einfassband-nach-mass` (in keiner Kollektion) - Testwerte danach wieder entfernen.
