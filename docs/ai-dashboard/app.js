...
function lieferantLinkZelle(p) {
  ...
  // Kurze Beschriftung, die Erklaerung steht im title - die Spalte ist eng.
  const lex = p.handle
    ? `<div style="margin-top:4px"><a class="btn btn-sm" href="#" data-lex-open="${esc(p.handle)}" title="Originalname, Artikelnummer, Farbnummer und Bestellweg im Lexikon">Im Lexikon ansehen</a></div>`
    : '';
...
function render() {
  ...
  if (more) {
    ...
    more.querySelector('summary').classList.toggle('current', ['freigaben', 'bereiche', 'insights', 'aktivitaet'].includes(state.route.view));
    more.open = false;
  }
