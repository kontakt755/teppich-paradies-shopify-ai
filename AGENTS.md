...
- Task-Klasse, Testtiefe, Review und Protected Actions werden getrennt abgeleitet. Modell, Effort, Reviewer und Corrector je Klasse stehen in `workflow/model-matrix.mjs` (A haiku ohne Modell-Review; B fable + Codex-Review; C fable high + Codex astra; D opus + Codex astra + Security-Review). Der Corrector ist nie schwächer als der Implementer; wiederholte Fehler eskalieren nach oben statt dasselbe Modell erneut zu versuchen. Ein Review ist verpflichtend vor einer geschützten externen Aktion.
