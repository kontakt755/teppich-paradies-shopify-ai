...
export async function runReviewCorrectionCycle({
  ...
  let candidate = await implement(task);
  ...
  for (let reviewRound = 1; reviewRound <= maxReviewRounds; reviewRound += 1) {
    ...
    const reviewResult = await review(task, candidate, { reviewRound, maxReviewRounds });
    ...
    candidate = mergeCorrectionCandidate(candidate, await correct(task, candidate, actionable, { reviewRound, maxReviewRounds }));
    if (candidate?.status !== 'PASS') {
      const status = ['PARKED', 'BLOCKED', 'SECURITY_STOP'].includes(candidate?.status) ? candidate.status : 'HARD_FAIL';
