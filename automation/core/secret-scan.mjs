...
const SAFE_ENV_TEMPLATE = /(^|\/)\.env\.(?:example|sample)$/i;
...
export function scanText({ file, text }) {
  ...
  const findings = [];
  if (SECRET_PATH.test(normalized) && !SAFE_ENV_TEMPLATE.test(normalized)) findings.push({ file: normalized, line: 1, rule: 'SECRET_FILE' });
  String(text).split(/\r?\n/).forEach((line, index) => {
